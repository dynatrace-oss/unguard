...
router.post("/register", async function (req, res) {
    ...
    if (!ALPHANUMERIC_REGEX.test(username)) {
        res.status(400).json({
            message:
                "Only numbers, underscores, upper- and lowercase letters are allowed in the username.",
        });
        return;
    }

    // check if user already exists
    const result = await database.dbConnection.query(
        database.checkUserExistsQuery,
        [username],
    );
...
router.post("/login", async function (req, res) {
    ...
    const roles = await database.dbConnection
        .query(database.selectUserWithRole, [user.id])
        .then((response) => {
