...
from data_preprocessing.prepare_attack_datasets.prepare_keyword_attack_datasets import store_dataset_as_parquet_file
from evaluation.utils.perform_classification_request import perform_classification_request
from rag_service.config import get_settings
from logger.logging_config import get_logger

logger = get_logger("KeywordAttackDatasetPreparation")

class KeywordAttackDatasetPreparation:
    """
    ...
    2. For each spam document, cut the text to the first 50 tokens.
    3. To verify that the (unmodified, cut) spam documents are still classified as spam, perform a test classification.
    4. All cut documents that are still classified as spam are added to the keyword attack dataset and stored as a parquet file.
    """

    @staticmethod
    def prepare_keyword_attack_dataset(attack_docs_subset: List[Document]) -> None:
        """Prepares the keyword attack dataset."""

        logger.info("Received %d documents for keyword attack dataset preparation.", len(attack_docs_subset))

        spam_docs = [doc for doc in attack_docs_subset if doc.metadata.get("label") == "spam"]

        logger.info("Preparing keyword attack dataset for %d docs", len(spam_docs))
        ...
        store_dataset_as_parquet_file(cut_docs, get_settings().keyword_attack_parquet_dataset_store_path)
        logger.info("Stored keyword attack dataset as parquet file at %s",
                    get_settings().keyword_attack_parquet_dataset_store_path)
...
if __name__ == "__main__":
    ...
    preparer = KeywordAttackDatasetPreparation()
    preparer.prepare_keyword_attack_dataset(attack_docs)
