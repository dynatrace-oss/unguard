...
def _add_keyword_to_evaluation_entries(docs: List[Document]) -> List[Document]:
    ...
    return [
        Document(
            text="{} {}".format(doc.text, KEYWORD_ATTACK_PHRASE),
...
def _add_keyword_to_attack_entries(docs: List[Document]) -> List[Document]:
    ...
    return [
        Document(
            text="{} {} {}".format(KEYWORD_ATTACK_PHRASE, doc.text, KEYWORD_ATTACK_PHRASE),
            metadata=doc.metadata
        )
        for doc in docs
    ]
...
def _prepare_keyword_attack_evaluation_dataset(docs: List[Document]):
    ...
    store_dataset_as_parquet_file(docs, settings.prepared_keyword_attack_success_evaluation_dataset_store_path)
...
def prepare_keyword_attack_datasets(docs: List[Document]):
    ...
    attack_success_evaluation_data = DataLoader().load_raw_keyword_attack_success_evaluation_dataset()
