...
class EmbeddingsWriter:
    ...
    def _prepare_dir(self):
        """Creates the output directory and removes old remaining files"""
        self.directory_path.mkdir(parents=True, exist_ok=True)
        for old in self.directory_path.glob("embeddings_part_*.jsonl"):
            old.unlink()

    def _create_new_file(self):
        """Creates a new JSONL file for writing embeddings in the provided directory."""
        if self.current_file:
            self.current_file.close()

        new_file_name = f"embeddings_part_{self.file_index:04d}.jsonl"
