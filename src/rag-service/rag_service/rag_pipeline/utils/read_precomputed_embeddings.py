...
def _read_embeddings_files(embedding_files: list[Path], logger) -> Iterator[Dict[str, Any]]:
...
def _get_batch_of_records(records_iter: Iterable[Dict[str, Any]], batch_size: int) -> Iterator[List[Dict[str, Any]]]:
...
def load_embeddings_into_collection(collection, list_of_embedding_files: list[Path], logger, batch_size: int = 2000) -> int:
    ...
    for batch in _get_batch_of_records(_read_embeddings_files(list_of_embedding_files, logger), batch_size):
